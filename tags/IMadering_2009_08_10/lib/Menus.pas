...
uses Controls, Forms, Consts, Themes, Registry, UxTheme;
...
{$ENDIF}
{$IFDEF LINUX}
  if Count = 0 then FWindow := WinUtils.AllocateHWnd(MainWndProc);
{$ENDIF}   
  inherited Add(Popup);
end;
...
{$IFDEF MSWINDOWS}   
  if Count = 0 then Classes.DeallocateHWnd(FWindow);
{$ENDIF}
{$IFDEF LINUX}
  if Count = 0 then WinUtils.DeallocateHWnd(FWindow);
{$ENDIF}   
end;
...
const
  Flags: array[Boolean, TPopupAlignment] of Word =
    ((TPM_BOTTOMALIGN + 10, TPM_CENTERALIGN, TPM_LEFTALIGN),
