...
type
  ...
  private
    ...
    FImageIndex1: TImageIndex;
    FImageIndex2: TImageIndex;
    FCaption: string;
    FData: Pointer;
    FHint: string;
    //--IMadering
    ...
    procedure SetImageIndex1(const Value: TImageIndex);
    procedure SetImageIndex2(const Value: TImageIndex);
  ...
  published
    ...
    property Idd: string read FIdd write FIdd;
    ...
    property XText: string read FXText write FXText;
    property InputText: string read FInputText write FInputText;
    property QuoteMsg: string read FQuoteMsg write FQuoteMsg;
    property Client: string read FClient write FClient;
    property TimeId: string read FTimeId write FTimeId;
    property UserClass: string read FUserClass write FUserClass;
    property IntIP: string read FIntIP write FIntIP;
    property IntPort: string read FIntPort write FIntPort;
    property ExtIP: string read FExtIP write FExtIP;
    property ConnFlag: string read FConnFlag write FConnFlag;
    property ProtoVer: string read FProtoVer write FProtoVer;
    property TimeReg: string read FTimeReg write FTimeReg;
    property CapsId: string read FCapsId write FCapsId;
    property Caps: string read FCaps write FCaps;
    property Capslist: string read FCapslist write FCapslist;
    property IconHash: string read FIconHash write FIconHash;
    property ConnTime: string read FConnTime write FConnTime;
    property TypingTime: Integer read FTypingTime write FTypingTime default 0;
    property Utf8Supported: Boolean read FUtf8Supported write FUtf8Supported default false;
    property History: string read FHistory write FHistory;
    property HistoryChange: Boolean read FHistoryChange write FHistoryChange default false;
    ...
    property ImageIndex1: TImageIndex read FImageIndex1 write SetImageIndex1 stored IsImageIndexStored;
    property ImageIndex2: TImageIndex read FImageIndex2 write SetImageIndex2 stored IsImageIndexStored;
...
begin
  ...
  //--Подсвечиваем заголовок выделенной группы
  if Category.GroupSelected then
  begin
    Canvas.Brush.Color := GetShadowColor(SourceColor, -75);
    with CategoryRealBounds do
      Canvas.FrameRect(Rect(Left + 1, Top + 1, Right, Bottom));
  ...
  begin
    ...
    begin
      ...
      if (bdsFocused in State) then
        ...
        EdgeColor := clWhite; //GetShadowColor(FillColor);
    end;
    ...
    begin
      ...
      if (FImages <> nil) and (Button.ImageIndex2 > -1) and
        (Button.ImageIndex2 < FImages.Count) then
      begin
        FImages.Draw(Canvas, TextLeft + Rect.Right - 29, ImgTop, Button.ImageIndex2);
      end;
      if (FImages <> nil) and (Button.ImageIndex1 > -1) and
        (Button.ImageIndex1 < FImages.Count) then
      begin
        TextLeft := TextLeft + 17;
        FImages.Draw(Canvas, TextLeft - 18, ImgTop, Button.ImageIndex);
        FImages.Draw(Canvas, TextLeft, ImgTop, Button.ImageIndex1);
      end
      else
...
begin
  ...
  begin
    ...
    begin
      ...
      Message.HintInfo.HideTimeout := MaxInt;
...
begin
  ...
  begin
    if FFocusedItem <> Value then UpdateButton(FFocusedItem);
    ...
    if Value <> nil then ScrollIntoView(Value);
    FSelectedItem := Value;
    if Value <> nil then FFocusedItem := Value;
    if FSelectedItem <> nil then
    begin
      UpdateButton(FSelectedItem);
      //if FSelectedItem.Category.Collapsed then
      ShareUpdateCategory(FSelectedItem.Category);
    end;
    if not (csDestroying in ComponentState) then DoSelectedItemChanged(FSelectedItem);
  end;
end;
...
procedure TBaseButtonItem.SetImageIndex1(const Value: TImageIndex);
begin
...
procedure TBaseButtonItem.SetImageIndex2(const Value: TImageIndex);
begin
...
begin
  ...
  if IsImageIndexLinked then FClient.ImageIndex1 := Value;
  if IsImageIndexLinked then FClient.ImageIndex2 := Value;
end;
