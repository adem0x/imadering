  ...
  if Assigned(ProfileForm) then
    begin
      if ProfileForm.Visible then
        begin
          ProfileForm.Hide;
          HideProfileInTray.Caption := RestoreProfileFromTrayStr;
...
begin
  ...
  if Visible then
    begin
      Hide;
      HideMainInTray1.Caption := RestoreFromTrayStr;
                              ...
                              $0021: begin
                                  ...
                                  if NextData(HexPkt, 2) = #$00#$01 then
                                    begin
                                      ...
                                      XLog('ICQ parsing | ' + Log_Icon_Hash + RN + ICQ_MyIcon_Hash);
                                    end;
                                end;
                            end;
                          end;
                                    ...
                                    begin
                                      ...
                                      case HexToInt(TLV) of
                                        $0008: begin // TLV с ошибкой авторизации
                                            Len := HexToInt(Text2Hex(NextData(HexPkt, 2)));
                                            DAShow(ErrorHead, ICQ_NotifyAuthCookieError(Text2Hex(NextData(HexPkt, Len))), EmptyStr, 134, 2,
                                              0);
                                            ICQ_GoOffline;
                                          end;
                                        ...
                                        $0006: begin // TLV с куком для коннекта к основному серверу
                                          ...
                                          begin
                                            Len := HexToInt(Text2Hex(NextData(HexPkt, 2)));
                                            XLog('ICQ parsing | ' + Log_Unk_Data + RN + 'TLV: ' + TLV + ' Value: ' + Trim
...
begin
  ...
  if HttpProxy_Enable then
    begin
      ...
      Http_data := 'CONNECT ' + Http_data + ' HTTP/1.0' + #13#10 + 'User-agent: Mozilla/4.08 [en] (WinNT; U)' + #13#10 + Http_login +
        #13#10;
...
begin
  ...
  HMutex := CreateMutex(0, True, 'IMadering client');
              ...
              if (SubItems[17] <> EmptyStr) and (IsWindow(StrToInt(SubItems[17]))) then
                begin
                  for I := 0 to Screen.FormCount - 1 do
                    begin
                      if Screen.Forms[I].Handle = StrToInt(SubItems[17]) then
                        begin
