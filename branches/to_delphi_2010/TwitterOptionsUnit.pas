...
uses
  ...
  JvPageList,
  JvExControls,
  JvSimpleXml, Mask, JvExMask, JvSpin;
...
resourcestring
  RS_InMessCount = 'InMessCount';
  RS_MyMessCount = 'MyMessCount';
              ...
              begin
                ...
                if XML_Node <> nil then
                  begin
                    ...
                    AccountEdit.Text := XML_Node.Properties.Value(RS_Login);
                    ...
                    Sub_Node := XML_Node.Items.ItemNamed[RS_InMessCount];
                    if Sub_Node <> nil then
                      InMessJvSpinEdit.Value := Sub_Node.IntValue;
                    // Количесво загружаемых моих сообщений
                    Sub_Node := XML_Node.Items.ItemNamed[RS_MyMessCount];
                    if Sub_Node <> nil then
      ...
      begin
        ...
        if Root <> nil then
          begin
            ...
            XML_Node.Properties.Add(RS_Login, AccountEdit.Text);
            ...
            // Количесво загружаемых входящих сообщений
            XML_Node.Items.Add(RS_InMessCount, InMessJvSpinEdit.Text);
            // Количесво загружаемых моих сообщений
            XML_Node.Items.Add(RS_MyMessCount, MyMessJvSpinEdit.Text);
            // --------------------------------------------------------------------------
            // Сохраняем файл
