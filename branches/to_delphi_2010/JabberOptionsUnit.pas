...
type
  TJabberOptionsForm = class(TForm)
    ...
    procedure JabberOptionButtonGroupButtonClicked
...
begin
  with SettingsXml do
    if OpenKey(StrSettingsJabberConnection, true) then
      try
        WriteBool(StrKeyUseCustomServerSettings,
          JUseCustomServerSettingsCheckBox.Checked);
    ...
    if OpenKey(StrSettingsJabberAccount, true) then
      try
        ...
        if SavePassCheckBox.Checked then
          WriteString(StrKeyPassword, EncryptString(PassEdit.Hint,
              PasswordByMac))
        else
...
begin
  ...
  JCustomServerHostEdit.Text := exNormalizeScreenName
    (JCustomServerHostEdit.Text);
  // Server Port
  JCustomServerPortEdit.Text := Trim(JCustomServerPortEdit.Text);
  JCustomServerPortEdit.Text := exNormalizeScreenName
    (JCustomServerPortEdit.Text);
...
begin
  with SettingsXml do
    if OpenKey(StrSettingsJabberConnection) then
      try
        JUseCustomServerSettingsCheckBox.Checked := ReadBool
          (StrKeyUseCustomServerSettings);
...
procedure TJabberOptionsForm.JUseCustomServerSettingsCheckBoxClick
  (Sender: TObject);
begin
