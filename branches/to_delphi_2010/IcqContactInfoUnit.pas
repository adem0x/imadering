...
uses
  ...
  rXML, UnitLogger, OverbyteIcsMimeUtils;
...
begin
  ...
  if FileExists(ProfilePath + AnketaFileName + ReqProto + '_' + ReqUIN +
      '.xml') then
  begin
  ...
  begin
    ...
    // Добавляем стили
    AddHTML(TopHTMLViewer, '<html><head>' + DetailsCSS +
        '<title>Details</title></head><body>', EmptyStr, false, false, true);
    ...
    // Загружаем аватар
    AvatarImage.Picture.LoadFromFile
...
var
  ...
  wCity, wState, wZip, wAddress, Company, Department, Position, wSite,
    wCountry, Occupation: string;
  Phone, Fax, Cellular, wPhone, wFax, HomePage, LastUpdateInfo: string;
  Int1, Int2, Int3, Int4, I1, I2, I3, I4, About: string;
  Lang1, Lang2, Lang3, Marital, Sexual, Height, Relig, Smok, Hair,
    Children: string;
...
begin
  ...
  AddHTML(TopHTMLViewer, '<html><head>' + DetailsCSS +
      '<title>Details</title></head><body>', EmptyStr, false, false, true);
  AddHTML(BottomHTMLViewer, '<html><head>' + DetailsCSS +
      '<title>Details</title></head><body>', EmptyStr, false, false, true);
  // Учётная запись контакта
  AddHTML(TopHTMLViewer, ICQAccountInfo + ' ' + ReqUIN, 'cbold');
  AddHTML(TopHTMLViewer, '<hr>', EmptyStr, false, true);
  ...
  with TrXML.Create() do
    try
      // Загружаем настройки
      LoadFromFile(ProfilePath + AnketaFileName + ReqProto + '_' + ReqUIN +
          '.xml');
      // Ник, Имя и фамилию
      if OpenKey('profile\name-info') then
        try
      ...
      if OpenKey('profile\home-info') then
        try
          ...
          if Assigned(IcqOptionsForm) then
            Country := IcqOptionsForm.CountryInfoComboBox.Items.Strings
              [IcqOptionsForm.CountryCodesComboBox.Items.IndexOf(Country)];
          if IsNotNull([Country, City]) then
          begin
            AddHTML(BottomHTMLViewer, InfoHomeL + ' ', 'cbold');
            AddHTML(BottomHTMLViewer, StrArrayToStr([Country, City]), 'cdef',
              true);
          end;
          ...
          // Вставляем разделитель
          if (IsNotNull([Country, City])) or (IsNotNull([Address, State, Zip]))
            then
            AddHTML(BottomHTMLViewer, '<hr>', EmptyStr, false, true);
        finally
          CloseKey();
      ...
      if OpenKey('profile\age-info') then
        try
          ...
          begin
            ...
            AddHTML(BottomHTMLViewer, iDay + '.' + iMonth + '.' + iYear,
              'cdef', true);
          end;
          // Вставляем разделитель
          if (Gender <> '0') or ((iDay <> '0') and (iMonth <> '0') and
              (iYear <> '0')) then
            AddHTML(BottomHTMLViewer, '<hr>', EmptyStr, false, true);
        finally
          CloseKey();
      ...
      if OpenKey('profile\orig-home-info') then
        try
          ...
          // Получаем текст страны из кода
          if Assigned(IcqOptionsForm) then
            oCountry := IcqOptionsForm.CountryInfoComboBox.Items.Strings
              [IcqOptionsForm.CountryCodesComboBox.Items.IndexOf(oCountry)];
          oCity := ReadString('city');
          oState := ReadString('state');
          if IsNotNull([oCountry, oCity]) then
          begin
            AddHTML(BottomHTMLViewer, InfoOHomeL + ' ', 'cbold');
            AddHTML(BottomHTMLViewer, StrArrayToStr([oCountry, oCity]), 'cdef',
              true);
          end;
      ...
      if OpenKey('profile\work-info') then
        try
          ...
          // Получаем текст страны из кода
          if Assigned(IcqOptionsForm) then
            wCountry := IcqOptionsForm.CountryInfoComboBox.Items.Strings
              [IcqOptionsForm.CountryCodesComboBox.Items.IndexOf(wCountry)];
          // Получаем текст занятия из кода
          if Assigned(IcqOptionsForm) then
            Occupation := IcqOptionsForm.CompanyProfInfoComboBox.Items.Strings
              [IcqOptionsForm.OccupationCodeComboBox.Items.IndexOf(Occupation)];
          if IsNotNull([wCountry, wCity]) then
          begin
            AddHTML(BottomHTMLViewer, InfoWorkL + ' ', 'cbold');
            AddHTML(BottomHTMLViewer, StrArrayToStr([wCountry, wCity]), 'cdef',
              true);
          end;
          ...
          // Вставляем разделитель
          if (IsNotNull([wCountry, wCity])) or
            (IsNotNull([wAddress, wState, wZip])) or
      ...
      if OpenKey('profile\interests-id-info') then
        try
          ...
          begin
            ...
            begin
              I1 := Interest1InfoComboBox.Items.Strings
                [InterestsCodesComboBox.Items.IndexOf(I1)];
              I2 := Interest1InfoComboBox.Items.Strings
                [InterestsCodesComboBox.Items.IndexOf(I2)];
              I3 := Interest1InfoComboBox.Items.Strings
                [InterestsCodesComboBox.Items.IndexOf(I3)];
              I4 := Interest1InfoComboBox.Items.Strings
                [InterestsCodesComboBox.Items.IndexOf(I4)];
            end;
          end;
      ...
      if OpenKey('profile\personal-x-info') then
        try
          ...
          begin
            with IcqOptionsForm do
            begin
              Marital := PersonalMaritalInfoComboBox.Items.Strings
                [MaritalCodesComboBox.Items.IndexOf(Marital)];
              Sexual := PersonalSexInfoComboBox.Items.Strings
                [SexCodesComboBox.Items.IndexOf(Sexual)];
              Relig := PersonalReligionInfoComboBox.Items.Strings
                [ReligionCodesComboBox.Items.IndexOf(Relig)];
              Smok := PersonalSmokInfoComboBox.Items.Strings
                [SmokCodesComboBox.Items.IndexOf(Smok)];
              Hair := PersonalHairColourInfoComboBox.Items.Strings
                [HairColourCodesComboBox.Items.IndexOf(Hair)];
            end;
          end;
          // Формируем отображение
          if IsNotNull([Marital, Sexual, Height, Relig, Smok, Hair, Children])
            then
          begin
            ...
            begin
              case StrToInt(Children) of
                ...
                8:
                  Children := '8';
                9:
                  Children := InfoChildrenL2;
                255:
                  Children := InfoChildrenL3;
              end;
              AddHTML(BottomHTMLViewer, InfoChildrenL1 + ' ', 'cbold');
              AddHTML(BottomHTMLViewer, Children, 'cdef', true);
            end;
            // Вставляем разделитель
            if (IsNotNull([Marital, Sexual, Relig, Smok, Hair])) or
              (Height <> '0') or (Children <> '0') then
              AddHTML(BottomHTMLViewer, '<hr>', EmptyStr, false, true);
          end;
        finally
          CloseKey();
      ...
      if OpenKey('profile\lang-info') then
        try
          ...
          begin
            with IcqOptionsForm do
            begin
              Lang1 := Lang1InfoComboBox.Items.Strings
                [LangsCodeComboBox.Items.IndexOf(Lang1)];
              Lang2 := Lang1InfoComboBox.Items.Strings
                [LangsCodeComboBox.Items.IndexOf(Lang2)];
              Lang3 := Lang1InfoComboBox.Items.Strings
                [LangsCodeComboBox.Items.IndexOf(Lang3)];
            end;
          end;
          ...
          begin
            ...
            AddHTML(BottomHTMLViewer, StrArrayToStr([Lang1, Lang2, Lang3]),
              'cdef', true);
            // Вставляем разделитель
            AddHTML(BottomHTMLViewer, '<hr>', EmptyStr, false, true);
          end;
  ...
  begin
    ...
    HoroImage.Picture.LoadFromFile
      (MyPath + 'Icons\' + CurrentIcons + '\horoscope.bmp');
    // Прокручиваем на картинку этого знака
    HoroImage.Canvas.CopyRect(Rect(0, 0, 32, 32), HoroImage.Canvas, Bounds
        (Horospope(StrToInt(iDay), StrToInt(iMonth)), 0, 32, 32));
  end
  else
    HoroImage.Picture.Assign(nil);
  // Загружаем аватар
