...
{$IFDEF LINUX}
  WinUtils,
{$ENDIF}
  Messages, Windows, SysUtils, CommCtrl, Classes, Forms, Controls, Menus,
...
type
  ...
  private
    ...
    procedure SetDescriptionTop(Value: string);
    procedure SetDescriptionBottom(Value: string);
    procedure SetTitleImage(Value: TImageIndex);
    procedure SetExtendedImage(Value: TImageIndex);
    procedure SetSubsetTitle(Value: string);
  protected
    function GetDisplayName: string; override;
  ...
  published
    ...
    property TopDescription: string read FDescriptionTop write SetDescriptionTop;
    property BottomDescription: string read FDescriptionBottom write SetDescriptionBottom;
    property TitleImage: TImageIndex read FTitleImage write SetTitleImage;
    property ExtendedImage: TImageIndex read FExtendedImage write SetExtendedImage;
...
{$ELSE}
  ActiveX,
{$IFEND}
  Printers, Consts, RTLConsts, ComStrs, ActnList, StdActns, Types,
        ...
        begin
          ...
          try
            ...
            begin
              ...
              LPoint := Types.SmallPoint(MsgPos);
              FTrackPos.X := LPoint.X;
              FTrackPos.Y := LPoint.Y;
...
begin
  ...
  case NodeDataType of
    ...
    begin
      Stream.ReadBuffer(Size, SizeOf(Size));
      Stream.ReadBuffer(Info2, SizeOf(TNodeDataInfo2));
      SetLength(LText, Info2.TextLen); // Info2.TextLen holds number of chars of Unicode string
      Stream.ReadBuffer(LText[1], Info2.TextLen * SizeOf(WideChar));
      Text := LText;
      ImageIndex := Info2.ImageIndex;
      SelectedIndex := Info2.SelectedIndex;
      StateIndex := Info2.StateIndex;
...
const
  TreeNodeStreamVersion    = $01;  // 32-bit struct size
  TreeNodeStreamVersion64  = $02;  // 64-bit struct size
  TreeNodeStreamVersion2   = $03;  // 32-bit struct size version 2
  TreeNodeStreamVersion264 = $04;  // 64-bit struct size version 2
...
{$IFEND}
  public
    ...
    procedure Delete(Index: Integer); override;
    procedure Insert(Index: Integer; const S: string); override;
    procedure LoadTreeFromStream(Stream: TStream; Encoding: TEncoding);
    procedure SaveTreeToStream(Stream: TStream; Encoding: TEncoding);
    property Owner: TTreeNodes read FOwner;
  end;
...
procedure TTreeStrings.LoadTreeFromStream(Stream: TStream; Encoding: TEncoding);
var
  List: TStringList;
...
procedure TCustomTreeView.LoadFromFile(const FileName: string; Encoding: TEncoding);
var
  Stream: TStream;
begin
  Stream := TFileStream.Create(FileName, fmOpenRead);
...
procedure TCustomTreeView.LoadFromStream(Stream: TStream; Encoding: TEncoding);
begin
  with TTreeStrings.Create(Items) do
    try
      LoadTreeFromStream(Stream, Encoding);
    finally
      Free;
  end;
end;
...
procedure TCustomTreeView.SaveToFile(const FileName: string; Encoding: TEncoding);
var
  Stream: TStream;
begin
  Stream := TFileStream.Create(FileName, fmCreate);
...
begin
  with TTreeStrings.Create(Items) do
    try
      SaveTreeToStream(Stream, Encoding);
    finally
      Free;
  end;
end;
...
{$ELSE}
      SendMessage(hwndFrom, TTM_ADJUSTRECT, Integer(True), Integer(@LItemRect));
{$IFEND}
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        SendMessage(Handle, TBM_SETSELSTART, Integer(False), FSelStart);
        SendMessage(Handle, TBM_SETSELEND, Integer(True), FSelEnd);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if (FStyle = pbstMarquee) and ThemeServices.ThemesAvailable and
...
begin
  ...
  if ThemeServices.ThemesAvailable and CheckWin32Version(5, 1) and
...
const
  ListItemStreamVersion   = $01; // 32-bit struct size
  ListItemStreamVersion64 = $02; // 64-bit struct size
  ListItemStreamVersion2   = $03; // 32-bit struct size version 2
  ListItemStreamVersion264 = $04; // 64-bit struct size version 2
...
begin
  ...
  else if StreamVersion = ListItemStreamVersion2 then
  begin
    Stream.ReadBuffer(Size, SizeOf(Integer));
    Stream.ReadBuffer(ItemCount, SizeOf(Integer));
    ...
    for I := 0 to ItemCount - 1 do
    begin
      Stream.ReadBuffer(ItemInfo2, SizeOf(TItemDataInfo2));
      with Add do
      begin
        ImageIndex := ItemInfo2.ImageIndex;
        OverlayIndex := ItemInfo2.OverlayIndex;
        ...
        // Read Caption
        SetLength(LWideStr, ItemInfo2.CaptionLen);
        Stream.ReadBuffer(LWideStr[1], ItemInfo2.CaptionLen * SizeOf(WideChar));
        Caption := LWideStr;

        // Read length of each SubItem in chars, followed by actual text
        for J := 0 to ItemInfo2.SubItemCount - 1 do
        begin
          Stream.ReadBuffer(Len, SizeOf(Byte));
          SetLength(LWideStr, Len);
          Stream.ReadBuffer(LWideStr[1], Len * SizeOf(WideChar));
...
begin
  ...
  if IntPtr.Size = 4 then
    ...
    Stream.WriteBuffer(ListItemStreamVersion264, SizeOf(Byte));
...
begin
  ...
  LItemCount := Count;
  LStreamVersion := ListItemStreamVersion2;
  Stream.WriteBuffer(LStreamVersion, SizeOf(Byte));
  Stream.WriteBuffer(Size, SizeOf(Integer));
...
{$IFEND}
    ...
    if not (((Message.Msg = WM_PAINT) or (Message.Msg = WM_ERASEBKGND)) and
...
{$IF DEFINED(CLR)}
      ...
      pszFooter := Marshal.StringToHGLobalAuto(Footer);
      pszSubtitle := Marshal.StringToHGLobalAuto(Subtitle);
      pszDescriptionTop := Marshal.StringToHGLobalAuto(TopDescription);
      pszDescriptionBottom := Marshal.StringToHGLobalAuto(BottomDescription);
      try
{$ELSE}
      cbSize := SizeOf(Group);
      ...
      pszDescriptionTop := PWideChar(UnicodeString(TopDescription));
      pszDescriptionBottom := PWideChar(UnicodeString(BottomDescription));
{$IFEND}
      cchHeader := Length(Header);
      cchFooter := Length(Footer);
      cchSubtitle := Length(Subtitle);
      cchDescriptionTop := Length(TopDescription);
      cchDescriptionBottom := Length(BottomDescription);

      Group.stateMask := 0;
      Group.state := 0;
      if lgsNormal in Groups.Items[AnIndex].State then
        Group.state := Group.state or LVGS_NORMAL;
      if lgsHidden in Groups.Items[AnIndex].State then
      ...
      begin
        mask := mask or LVGF_TITLEIMAGE or LVGF_EXTENDEDIMAGE;

        if Subtitle <> '' then
          mask := mask or LVGF_SUBTITLE;
        if TopDescription <> '' then
          mask := mask or LVGF_DESCRIPTIONTOP;
        if BottomDescription <> '' then
          mask := mask or LVGF_DESCRIPTIONBOTTOM;

        if lgsNoHeader in Groups.Items[AnIndex].State then
          Group.state := Group.state or LVGS_NOHEADER;
        if lgsCollapsible in Groups.Items[AnIndex].State then
          Group.state := Group.state or LVGS_COLLAPSIBLE;
        if lgsCollapsed in Groups.Items[AnIndex].State then
          Group.state := Group.state or LVGS_COLLAPSED;
        if lgsFocused in Groups.Items[AnIndex].State then
          Group.state := Group.state or LVGS_FOCUSED;
        if lgsSelected in Groups.Items[AnIndex].State then
          Group.state := Group.state or LVGS_SELECTED;
        if lgsSubseted in Groups.Items[AnIndex].State then
          Group.state := Group.state or LVGS_SUBSETED;
        if lgsSubSetLinkFocused in Groups.Items[AnIndex].State then
          Group.state := Group.state or LVGS_SUBSETLINKFOCUSED;
...
begin
  ...
  if csDesigning in ComponentState then
    ...
    if Style in [tbsSeparator, tbsDivider] then
      ...
      if (FToolBar.Flat or ThemeServices.ThemesEnabled) and not Down then
      begin
...
begin
  if (FToolBar <> nil) and not FEnableDropdown then
    FToolBar.Perform(TB_ENABLEBUTTON, Index, Longint(Ord(Enabled)));
end;
  ...
  begin
    ...
    if not Enabled and (FToolbar <> nil) then
      if FEnableDropdown then
        FToolBar.Perform(TB_ENABLEBUTTON, Index, Longint(Ord(True)))
      else
        FToolbar.Perform(TB_ENABLEBUTTON, Index, Longint(Ord(False)));
  end;
end;
...
{$IFEND}
  ...
  FTransparent := ThemeServices.ThemesEnabled;
...
begin
  ...
  begin
    Style := Style or DefaultStyles or FlatStyles[FFlat] or ListStyles[FList] or
      ...
      FlatOnXp[ThemeServices.ThemesEnabled];
...
begin
  ...
  begin
    ...
    Perform(TB_SETEXTENDEDSTYLE, 0, Longint(CurStyle));
  end;
...
begin
  ...
  begin
    FButtonHeight := Value;
    if (ThemeServices.ThemesEnabled = True) and Showing then
      RecreateWnd;
    RecreateButtons;
  end;
end;
...
begin
  ...
  if ThemeServices.ThemesEnabled and HandleAllocated and HasDropDownButton then
    RecreateWnd
  else
    RecreateButtons;
end;
...
{$ELSE}
  Result := inherited Perform(Msg, WParam, Windows.LPARAM(@LParam));
{$IFEND}
end;
...
{$IFEND}
  SendStructMessage(Handle, _TB_SAVERESTORE, Integer(Save), SP)
end;
...
begin
  Result := inherited IsDropdownMenuLinked and
    (FClient.DropdownMenu = (Action as TCustomControlAction).DropdownMenu);
end;

function TToolButtonActionLink.IsEnableDropdownLinked: Boolean;
begin
  Result := inherited IsEnableDropdownLinked and
    (FClient.EnableDropdown = (ACtion as TCustomControlAction).EnableDropdown);
end;
...
begin
  ...
  begin
    ...
    DateTimeToSystemTime(TruncMax, STA[1]);
  end;
  if HandleAllocated then
...
begin
  if IsImagesLinked then
    with FClient as TCustomListView do
      TCustomListView(FClient).SmallImages := Value
end;
...
procedure TListGroup.SetDescriptionTop(Value: string);
begin
...
procedure TListGroup.SetDescriptionBottom(Value: string);
begin
  if FDescriptionBottom <> Value then
  begin
...
procedure TListGroup.SetExtendedImage(Value: TImageIndex);
begin
  if FExtendedImage <> Value then
  begin
...
procedure TListGroup.SetSubsetTitle(Value: string);
begin
  if FSubsetTitle <> Value then
  begin
    FSubsetTitle := Value;
    if not (csLoading in TListGroups(Collection).Owner.ComponentState) then
      TListGroups(Collection).Owner.UpdateGroups;
