  ...
  begin
    ...
    for I := 1 to Length(aText) do
      case aText[I] of
        ...
        '&':
          begin
            Move(PChar('&amp;')^, Result[J], 5);
            Inc(J, 5)
          end;
        '"':
          begin
            Move(PChar('&quot;')^, Result[J], 6);
            Inc(J, 6)
          end;
      else
        begin
...
begin
  case v.VType of
    ...
    varOleStr:
      Result := v.VOleStr;
    varBoolean:
      Result := BoolStr[v.VBoolean = True];
    varShortInt:
...
begin
  ...
  begin
    I := 1;
    while (I <= Length(aData)) and (CharInSet(aData[I], [#10, #13, #9, ' '])) do
      Inc(I);
...
function TXmlNode.GetBoolAttr(aNameID: Integer; aDefault: Boolean): Boolean;
var
  aData: PXmlAttrData;
begin
  aData := FindAttrData(aNameID);
...
function TXmlNode.GetFloatAttr(aNameID: Integer; aDefault: Double): Double;
var
  aData: PXmlAttrData;
begin
  aData := FindAttrData(aNameID);
  ...
  begin
    ...
    begin
      ...
      begin
        Inc(FGetXMLIntend);
        try
          aChildsXML := FChilds.Get_XML finally Dec(FGetXMLIntend)
        end
      end
      else if VarIsEmpty(FData) then
        aChildsXML := ''
...
{$ENDIF}
  ...
  begin
    ...
    begin
      aChild := aChilds.FItems[I];
      if aChild.Get_NodeType = NODE_ELEMENT then
      begin
        Result := aChild.AsElement;
        Exit
      end
  ...
  begin
    RemoveAllChilds;
    RemoveAllAttrs;
    if aStream.Size > BinXmlSignatureSize then
    begin
