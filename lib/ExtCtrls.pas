...
{$IF DEFINED(LINUX)}
  WinUtils,
{$IFEND}
  Messages, Windows, SysUtils, Classes, Contnrs, Types,  
...
type
  ...
  private
    ...
    FxImageIndex: TImageIndex; // добавил
    ...
    procedure SetxImageIndex(const Value: TImageIndex); // добавил
    procedure SetExpandedHeight(const Value: Integer); // добавил
...
{$IFEND}
  ...
  public
    ...
    property xImageIndex: TImageIndex read FxImageIndex write SetxImageIndex default -1; // добавил
    property ExpandedHeight: Integer read FExpandedHeight write SetExpandedHeight default 90; // добавил
...
{$IFEND}
  ...
  public
    ...
    function GetAllHeaderHeight: integer; // добавил
    procedure SetOpenPanelHeight; // добавил
...
{$IFEND}
  Consts, Dialogs, Themes, Math, UxTheme, DwmApi, CommCtrl;
...
begin
  ...
  end else if FWindowHandle <> 0 then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        R := Rect(ARect.Left + Offset, ARect.Top + 2,
...
function TEditButton.GetImages: TImageList;
begin
...
procedure TCustomButtonedEdit.SetImages(const Value: TImageList);
begin
...
{$ELSE}
    ...
    if not CheckWin32Version(6, 0) then
      // Windows versions prior to Vista don't have additional fields
      Dec(FData^.cbSize, SizeOf(FData^.guidItem) + SizeOf(FData^.hBalloonIcon));
    FData^.Wnd := AllocateHwnd(WindowProc);
...
const
  Offset = 4;
  ChevronState: array[THeaderState] of array[Boolean] of Integer =
    ((EBHP_NORMAL, EBNGE_NORMAL), (EBHP_HOT, EBNGE_HOT), (EBHP_PRESSED, EBNGE_PRESSED));
  ChevronPart: array[Boolean] of Integer = (EBP_NORMALGROUPCOLLAPSE, EBP_NORMALGROUPEXPAND);
var
  LSize: TSize;
...
begin
  ...
  if (FPanelGroup.HeaderStyle = hsThemed) and ThemeServices.ThemesEnabled then
  begin
    GetThemePartSize(ThemeServices.Theme[teExplorerBar], Canvas.Handle,
      ChevronPart[FCollapsed], ChevronState[FHeaderState, FCollapsed], nil,
      TS_DRAW, LSize);
    LHeight := LSize.cy;
    LWidth := LSize.cx;
  end
  ...
  LeftOffset := 0;
  TopOffset := ((HeaderHeight - LHeight) div 2) - 0; // изменил
  ...
  if (FPanelGroup.HeaderStyle = hsThemed) and ThemeServices.ThemesEnabled then
    Inc(FGlyphRect.Top); // Themed chevron needs to be pushed down by 1

  // Calculate text rect
  FTextRect := Rect(Offset + (FGlyphRect.Right - FGlyphRect.Left) + Offset + 18, 0, // изменил
...
begin
  ...
  begin
    ...
    try
      ...
      // изменил
      with FPanelGroup.VertScrollBar do
      begin
...
begin
  ...
  LRect.Bottom := HeaderHeight; //GetCollapsedHeight; // изменил
  ...
  //Inc(LRect.Left, 5);
  //Dec(LRect.Right, 5);
  ACanvas.Pen.Color := GetShadowColor(FPanelGroup.GradientColor, - 40);
  ...
  // право
  //ACanvas.MoveTo(LRect.Right - 1, 0);
  //ACanvas.LineTo(LRect.Right - 1, LRect.Top);

  // лево
  //ACanvas.MoveTo(LRect.Left, 0);
  //ACanvas.LineTo(LRect.Left, LRect.Top);

  // верх
  //ACanvas.MoveTo(LRect.Left, 0);
  //ACanvas.LineTo(LRect.Right, 0);
...
var
  ...
  begin
    GradientFillCanvas(ACanvas, FPanelGroup.GradientBaseColor,
      FPanelGroup.GradientColor, LRect, FPanelGroup.GradientDirection);
  end;
  ...
  case FPanelGroup.HeaderStyle of
    ...
    hsThemed:
      ...
      begin
        ACanvas.Brush.Color := Color;
        ACanvas.FillRect(LRect);
        DrawThemeBackground(ThemeServices.Theme[teExplorerBar],
          ACanvas.Handle, EBP_NORMALGROUPHEAD, 0, LRect, nil)
      end
      else
        DrawGradientHeader;
    hsImage:
...
const
  ChevronDirection: array[TAlignment] of array[Boolean] of TScrollDirection =
    ((sdDown, sdRight), (sdDown, sdLeft), (sdDown, sdRight));
  ChevronState: array[THeaderState] of array[Boolean] of Integer =
    ((EBNGC_NORMAL, EBNGE_NORMAL), (EBNGC_HOT, EBNGE_HOT), (EBNGC_PRESSED, EBNGE_PRESSED));
  ChevronPart: array[Boolean] of Integer = (EBP_NORMALGROUPCOLLAPSE, EBP_NORMALGROUPEXPAND);
var
  LPoint: TPoint;
  LIndex: Integer;
begin
  if (FPanelGroup.HeaderStyle = hsThemed) and ThemeServices.ThemesEnabled then
  begin
    DrawThemeBackground(ThemeServices.Theme[teExplorerBar], ACanvas.Handle,
      ChevronPart[FCollapsed], ChevronState[FHeaderState, FCollapsed], FGlyphRect, nil);
  end
  else
  begin
    if UsingImageList then
    begin
      ...
      // добавил
      if FxImageIndex <> -1 then
        FPanelGroup.Images.Draw(ACanvas, FGlyphRect.Left + 18, FGlyphRect.Top, FxImageIndex);
    ...
    begin
      // draw chevron
      case FHeaderState of
        hsNormal: ACanvas.Pen.Color := FPanelGroup.ChevronColor;
        hsHot, hsPressed: ACanvas.Pen.Color := FPanelGroup.ChevronHotColor;
      end;
...
begin
  ...
  begin
    // добавил
    FPanelGroup.CollapseAll;
    H := FPanelGroup.GetAllHeaderHeight;
...
// добавил
procedure TCustomCategoryPanel.SetxImageIndex(const Value: TImageIndex);
begin
  if Value <> FxImageIndex then
  begin
    FxImageIndex := Value;
    if not (csLoading in ComponentState) and (PanelGroup.HeaderStyle = hsImage) and
       not Collapsed and (FHeaderState = hsPressed) then
...
// добавил
procedure TCustomCategoryPanel.SetExpandedHeight(const Value: Integer);
begin
...
begin
  inherited;
  if FCollapsed then
    LHeight := GetCollapsedHeight - 0 // изменил
  else
...
begin
  inherited CreateParams(Params);
  {with Params do
    Style := Style or WS_BORDER;} // изменил
end;
...
function TCustomCategoryPanelGroup.GetAllHeaderHeight: integer;
var
  I, H: Integer;
begin
  H := - HeaderHeight;
  for I := FPanels.Count - 1 downto 0 do
  begin
    if TCustomCategoryPanel(FPanels[I]).Visible then
      H := H + HeaderHeight;
  end;
  H := Height - H;
  Result := H;
...
procedure TCustomCategoryPanelGroup.SetOpenPanelHeight;
var
  I: Integer;
begin
  for I := FPanels.Count - 1 downto 0 do
  begin
    if not TCustomCategoryPanel(FPanels[I]).Collapsed then
      TCustomCategoryPanel(FPanels[I]).ExpandedHeight := GetAllHeaderHeight;
  end;
end;
...
procedure TCustomCategoryPanelGroup.SetImages(const Value: TImageList);
var
  I: Integer;
begin
  if Value <> FImages then
