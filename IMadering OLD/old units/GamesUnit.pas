...
type
  TGamesForm = class(TForm)
    ...
    procedure GamesClientDocBegin(Sender: TObject);
    procedure GamesClientDocData(Sender: TObject; Buffer: Pointer; Len: Integer);
    procedure GamesClientRequestDone(Sender: TObject; RqType: THttpRequest; ErrCode: Word);
    ...
    procedure GamesClientSessionClosed(Sender: TObject);
    procedure GamesClientSocksConnected(Sender: TObject; ErrCode: Word);
    procedure GamesClientSocksError(Sender: TObject; Error: Integer; Msg: string);
    ...
    procedure GamesClientCookie(Sender: TObject; const Data: string; var Accept: Boolean);
...
begin
  ...
  SettingsForm.ApplyProxyHttpClient(GamesClient);
end;
...
procedure TGamesForm.GamesClientCookie(Sender: TObject; const Data: string; var Accept: Boolean);
begin
  // Управляем кукие
  //XLog(GamesClient.Name + ' ' + Log_Get + ' ' + C_Cookie, Data, C_HTTP);
end;

procedure TGamesForm.GamesClientDocBegin(Sender: TObject);
begin
  // Создаём блок памяти для приёма http данных
  GamesClient.RcvdStream := TMemoryStream.Create;
end;
...
procedure TGamesForm.GamesClientDocData(Sender: TObject; Buffer: Pointer; Len: Integer);
begin
  // Отображаем процесс получения данных
  if GamesClient.ContentLength > -1 then
  begin
    LoadSizeLabel.Caption := Format(Lang_Vars[64].L_S, [FloatToStrF(GamesClient.RcvdCount / 1000, FfFixed, 7, 1)]);
    LoadGameProgressBar.Max := GamesClient.ContentLength;
    LoadGameProgressBar.Position := GamesClient.RcvdCount;
  end;
  // Обновляем форму и контролы чтобы видеть изменения
  Update;
  // Если был активирован аборт сессии, то выходим и отключаем сокет
  if GamesClient.Tag = 2 then
  begin
    GamesClient.CloseAsync;
    GamesClient.Abort;
  end;
end;
...
procedure TGamesForm.GamesClientRequestDone(Sender: TObject; RqType: THttpRequest; ErrCode: Word);
label
  X;
var
  List: TStringList;
...
begin
  try
    ...
    if GamesClient.RcvdStream <> nil then
    begin
      try
        // Увеличиваем статистику входящего трафика
        V_TrafRecev := V_TrafRecev + GamesClient.RcvdCount;
        V_AllTrafRecev := V_AllTrafRecev + GamesClient.RcvdCount;
        if Assigned(TrafficForm) then
          MainForm.Traffic_MenuClick(nil);
        // Обнуляем позицию начала чтения в блоке памяти
        GamesClient.RcvdStream.Position := 0;
        ForceDirectories(V_ProfilePath + C_GamesFolder);
        case GamesClient.Tag of // Определяем выполнение задания для данных по флагу
          0:
            begin
              ...
              try
                ...
                List.LoadFromStream(GamesClient.RcvdStream);
          ...
          1:
            begin
              ...
              try
                ...
                GFile.LoadFromStream(GamesClient.RcvdStream);
        ...
        GamesClient.RcvdStream.Free;
        GamesClient.RcvdStream := nil;
      end;
    end;
  except
    on E: Exception do
...
begin
  ...
  GamesClient.URL := C_GoogleCodeURL + C_GamesCatalogFileName;
  GamesClient.GetASync;
end;
...
begin
  ...
  GamesClient.URL := CatalogButtonGroup.Items[CatalogButtonGroup.ItemIndex].Hint;
...
procedure TGamesForm.GamesClientSessionClosed(Sender: TObject);
var
  S: string;
begin
  // Обрабатываем возможные ошибки в работе http сокета
  if (GamesClient.StatusCode = 0) or (GamesClient.StatusCode >= 400) then
  begin
    S := Format(ErrorHttpClient(GamesClient.StatusCode), [(Sender as THttpCli).Name, ' ']);
    FlashPanel.Caption := S;
  end;
end;

procedure TGamesForm.GamesClientSocksConnected(Sender: TObject; ErrCode: Word);
begin
...
procedure TGamesForm.GamesClientSocksError(Sender: TObject; Error: Integer; Msg: string);
begin
  // Если возникла ошибка, то сообщаем об этом
  if Error <> 0 then
  begin
    DAShow(Lang_Vars[17].L_S, Lang_Vars[23].L_S + C_RN + Msg + C_RN + Format(Lang_Vars[27].L_S, [Error]) + C_RN + '[ ' + Lang_Vars[94].L_S + ':' + (Sender as THttpCli).name + ' ]', EmptyStr, 134, 2, 0);
  end;
end;
