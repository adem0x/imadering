...
{ -----------------------------------------------------------------------------
  ...
  ----------------------------------------------------------------------------- }
// $Id: JvListView.pas 130 2009-08-09 17:59:21Z obones $
...
type
  ...
  TJvOnProgress = procedure(Sender: TObject; Progression, Total: Integer)
    of object;
  TJvListViewCompareGroupEvent = procedure(Sender: TObject;
    Group1, Group2: TJvListViewGroup; var Compare: Integer) of object;

  TJvListItems = class(TListItems, IJvAppStorageHandler,
  ...
  private
    ...
    procedure TileColumnsChange(Sender: TObject; Item: Integer;
  ...
  published
    property SortMethod
      : TJvSortMethod read GetSortMethod write SetSortMethod default
      smAutomatic;
    property UseParentSortMethod
      : Boolean read FUseParentSortMethod write FUseParentSortMethod default
      True;
    property HeaderImagePosition
      : TJvHeaderImagePosition read GetHeaderImagePosition write
      SetHeaderImagePosition default hipLeft;
    property UseParentHeaderImagePosition
      : Boolean read FUseParentHeaderImagePosition write
      SetUseParentHeaderImagePosition default True;
  end;
  ...
  public
    ...
    property Items[Index: Integer]
      : TJvListExtendedColumn read GetItem write SetItem; default;
  end;
  ...
  published
    ...
    property HeaderAlignment: TAlignment read FHeaderAlignment write
      SetHeaderAlignment default taLeftJustify;
  end;
  ...
  public
    ...
    property Items[Index: Integer]
      : TJvListViewGroup read GetItem write SetItem; default;
  ...
  public
    ...
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
  published
    property BorderSize: TJvGroupsPropertiesBorderRect read FBorderSize write
      SetBorderSize;
    property HeaderColor
      : TColor read FHeaderColor write SetHeaderColor default clBlack;

    // Note that BorderColor is currently ignored by the Win32 API
    property BorderColor
      : TJvGroupsPropertiesBorderColors read FBorderColor write SetBorderColor;
  end;
  ...
  public
    ...
    property OnChange: TNotifyEvent read FOnChange write FOnChange;
  published
    property TileSizeKind: TJvTileSizeKind read FTileSizeKind write
      SetTileSizeKind default tskAutoSize;
    property TileSize: TJvSize read FTileSize write SetTileSize;
    property SubLinesCount
      : Integer read FSubLinesCount write SetSubLinesCount default 1;
  ...
  private
    ...
    procedure WMAutoSelect(var Msg: TMessage);
    message WM_AUTOSELECT;
  ...
  protected
    ...
    procedure LVMDeleteColumn(var Msg: TMessage);
    message LVM_DELETECOLUMN;
    procedure LVMInsertColumn(var Msg: TMessage);
    message LVM_INSERTCOLUMN;
    procedure LVMSetColumn(var Msg: TMessage);
    ...
    function IsCustomDrawn(Target: TCustomDrawTarget; Stage: TCustomDrawStage)
      : Boolean; override;
    function CustomDraw(const ARect: TRect; Stage: TCustomDrawStage): Boolean;
      override;
  ...
  public
    ...
    function ShowInsertMark(ItemIndex: Integer; Position: TJvInsertMarkPosition)
      : Boolean;
    function HideInsertMark: Boolean;
    function GetInsertMarkPosition(const X, Y: Integer; var ItemIndex: Integer;
      var Position: TJvInsertMarkPosition): Boolean;

    property ItemPopup[Item: TListItem]
      : TPopupMenu read GetItemPopup write SetItemPopup;
  ...
  published
    ...
    property HeaderImagePosition
      : TJvHeaderImagePosition read FHeaderImagePosition write
      SetHeaderImagePosition default hipLeft;
    property HeaderImages: TCustomImageList read FHeaderImages write
      SetHeaderImages;
    property SortMethod
      : TJvSortMethod read FSortMethod write FSortMethod default smAutomatic;
    property SortOnClick
      : Boolean read FSortOnClick write FSortOnClick default True;
    property SmallImages write SetSmallImages;
    property AutoClipboardCopy
      : Boolean read FAutoClipboardCopy write FAutoClipboardCopy default True;
    property GroupView
      : Boolean read FGroupView write SetGroupView default False;
    property Groups: TJvListViewGroups read FGroups write SetGroups;
    property GroupsProperties: TJvGroupsProperties read FGroupsProperties write
      SetGroupsProperties;
    property TileViewProperties
      : TJvTileViewProperties read FTileViewProperties write
      SetTileViewProperties;
    property InsertMarkColor
      : TColor read FInsertMarkColor write SetInsertMarkColor default clBlack;

    property ViewStylesItemBrush: TJvViewStyles read FViewStylesItemBrush write
      SetViewStylesItemBrush default ALL_VIEW_STYLES;
    property ViewStyle
      : TJvViewStyle read FViewStyle write SetJvViewStyle default vsIcon;

    property OnAutoSort
      : TJvListViewColumnSortEvent read FOnAutoSort write FOnAutoSort;
    property OnHorizontalScroll
      : TNotifyEvent read FOnHorizontalScroll write FOnHorizontalScroll;
    property OnLoadProgress
      : TJvOnProgress read FOnLoadProgress write FOnLoadProgress;
    property OnSaveProgress
      : TJvOnProgress read FOnSaveProgress write FOnSaveProgress;
    property OnVerticalScroll
      : TNotifyEvent read FOnVerticalScroll write FOnVerticalScroll;
    property OnCompareGroups
      : TJvListViewCompareGroupEvent read FOnCompareGroups write
      FOnCompareGroups;
    ...
    property ExtendedColumns
      : TJvListExtendedColumns read FExtendedColumns write SetExtendedColumns;
  end;
{$IFDEF UNITVERSIONING}

const
  UnitVersioning: TUnitVersionInfo = (RCSfile:
      '$URL: https://jvcl.svn.sourceforge.net:443/svnroot/jvcl/trunk/jvcl/run/JvListView.pas $'; Revision: '$Revision: 130 $';
    Date: '$Date: 2009-08-09 19:59:21 +0200 (dim., 09 aoГ»t 2009) $';
    LogPath: 'JVCL\run');
{$ENDIF UNITVERSIONING}
...
type
  ...
  TFNLVGROUPCOMPARE = function(Group1_ID: Integer; Group2_ID: Integer;
...
const
  ...
  AlignmentToLVGA: array [TAlignment] of Integer = (LVGA_HEADER_LEFT,
    LVGA_HEADER_RIGHT, LVGA_HEADER_CENTER);
  TileSizeKindToLVTVIF: array [TJvTileSizeKind] of Integer =
    (LVTVIF_AUTOSIZE, LVTVIF_FIXEDWIDTH, LVTVIF_FIXEDHEIGHT, LVTVIF_FIXEDSIZE);
  InsertMarkPositionToLVIM: array [TJvInsertMarkPosition] of Integer =
    (0, LVIM_AFTER);
...
procedure TJvListItems.ReadFromAppStorage(AppStorage: TJvCustomAppStorage;
  const BasePath: string);
begin
  BeginUpdate;
  try
...
procedure TJvListItems.WriteToAppStorage(AppStorage: TJvCustomAppStorage;
  const BasePath: string);
begin
  AppStorage.WriteList(BasePath, Self, Count, WriteListItem, DeleteListItem,
    cItem);
end;
...
begin
  if List is TJvListItems then
    if Assigned(TJvListItems(List)[Index]) then
      Sender.WritePersistent
        (Sender.ConcatPaths([Path, ItemName + IntToStr(Index)]), TPersistent
          (TJvListItems(List)[Index]));
end;
...
begin
  ...
  begin
    ...
    FUseParentHeaderImagePosition := TJvListExtendedColumn(AValue)
      .UseParentHeaderImagePosition;
  end
  else
...
begin
  if (TJvListExtendedColumns(Collection).Owner is TJvListView)
    and UseParentHeaderImagePosition then
    Result := TJvListView(TJvListExtendedColumns(Collection).Owner)
      .HeaderImagePosition
  else
...
begin
  ...
  begin
    TJvListView(TJvListExtendedColumns(Collection).Owner).DoHeaderImagesChange
      (Self);
  end;
end;

procedure TJvListExtendedColumn.SetSortMethod(const Value: TJvSortMethod);
begin
  FSortMethod := Value;
...
function TJvListExtendedColumns.GetItem(Index: Integer): TJvListExtendedColumn;
begin
...
var
  ...
  begin
    ...
    if Parm.Index < TJvListView(Parm.Sender).ExtendedColumns.Count then
      SortKind := TJvListView(Parm.Sender).ExtendedColumns[Parm.Index]
        .SortMethod
    else
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Li := Items.Add;
        TmpStr := AnsiExtractQuotedStr(Start, '"');
        // this moves the PChar pointer
        Li.Caption := TmpStr;
      end
      else
      begin
...
begin
  ...
  try
    ...
    S.SaveToFile(FileName, TEncoding.Unicode);
  finally
    Items.EndUpdate;
    S.Free;
  end;
  ...
  with TStringList.Create do
    try
      ...
      begin
        ...
        begin
          Columns[I].Width := StrToIntDef
            (Copy(st, J + 1, Length(st)), Columns[I].Width);
          st := Copy(st, 1, J - 1);
        end;
      ...
      ListView_SetColumnOrderArray(Columns.Owner.Handle, Columns.Count,
        @Res[0]);
    finally
      Free;
    end;
...
begin
  if ReplaceImageListReference(Self, Value, FHeaderImages, FImageChangeLink)
    then
    UpdateHeaderImages(ListView_GetHeader(Handle));
end;

procedure TJvListView.SetExtendedColumns(const Value: TJvListExtendedColumns);
begin
...
begin
  ...
  if Operation = opRemove then
    if AComponent = HeaderImages then
      HeaderImages := nil
    else if not(csDestroying in ComponentState) and (AComponent is TPopupMenu)
      then
      for I := 0 to Items.Count - 1 do
...
function TJvListView.ShowInsertMark(ItemIndex: Integer;
  Position: TJvInsertMarkPosition): Boolean;
var
  Infos: TLVINSERTMARK;
begin
  ZeroMemory(@Infos, sizeof(Infos));
...
begin
  ...
  Infos.cbSize := sizeof(Infos);
  Result := Bool(SendMessage(Handle, LVM_INSERTMARKHITTEST, WParam(@Point),
      lParam(@Infos)));
  if Result then
  begin
...
function TJvListView.IsCustomDrawn(Target: TCustomDrawTarget;
...
begin
  ...
  Result := inherited IsCustomDrawn(Target, Stage) or
    ((Stage in [cdPrePaint, cdPostPaint]) and (Picture.Graphic <> nil)
      and not Picture.Graphic.Empty) or ((Stage in [cdPrePaint, cdPostPaint])
      and ((Target = dtItem) or (Target = dtSubItem)));
end;

function TJvListView.CustomDraw(const ARect: TRect; Stage: TCustomDrawStage)
  : Boolean;
var
...
begin
  ...
  if Result and (Stage = cdPrePaint) and (FPicture <> nil) and
    (FPicture.Graphic <> nil) and not FPicture.Graphic.Empty and
    (FPicture.Graphic.Width > 0) and (FPicture.Graphic.Height > 0) then
  begin
    Bmp := TBitmap.Create;
    try
      ...
      begin
        ...
        TopOffset := ListView_GetTopIndex(Handle) *
      ...
      BitBlt(Canvas, 0, 0, ClientWidth, ClientHeight, Bmp.Canvas, 0, 0,
        SRCCOPY);
...
function TJvListView.CustomDrawItem(Item: TListItem; state: TCustomDrawState;
  Stage: TCustomDrawStage): Boolean;
begin
...
procedure TJvListView.SetHeaderImagePosition
  (const Value: TJvHeaderImagePosition);
begin
...
procedure TJvListViewGroup.UpdateGroupProperties
  (const NewGroupId: Integer = -1);
var
  GroupInfo: TLVGROUP;
  List: TJvListView;
begin
...
function LVGroupCompare(Group1_ID: Integer; Group2_ID: Integer; pvData: Pointer)
  : Integer; stdcall;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      SendMessage(List.Handle, LVM_INSERTGROUPSORTED, WParam(@GroupSortedInfo),
        0);
    end
    else
    begin
...
begin
  ...
  if Assigned(List) then
  begin
    SendMessage(List.Handle, LVM_SORTGROUPS, WParam(@LVGroupCompare), lParam
        (Self));
  end;
end;
...
procedure TJvGroupsProperties.SetBorderColor
  (const Value: TJvGroupsPropertiesBorderColors);
begin
  FBorderColor.Assign(Value);
end;

procedure TJvGroupsProperties.SetBorderSize
  (const Value: TJvGroupsPropertiesBorderRect);
begin
