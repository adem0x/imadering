...
{$ELSE}
  Registry, Character,
{$IFEND}
  Types, Controls, Forms, Consts, Themes, UxTheme;
...
type
  TIterator = function (MenuItem: TMenuItem): Boolean;
...
begin
  Result := inherited IsCaptionLinked and
    SameCaption(FClient.Caption, (Action as TCustomAction).Caption);
end;
...
{$IFEND}
      ...
      if (Parent.Owner is TMainMenu) and CheckWin32Version(6, 0) and
         ThemeServices.ThemesAvailable then // Vista main menu items only
...
var
  ...
  const
    MenuStates: array[Boolean, Boolean] of Integer =
      ((MBI_NORMAL, MBI_PUSHED), (MBI_HOT, MBI_PUSHED));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          GetThemeColor(ThemeServices.Theme[teMenu], MENU_BARITEM,
            TopLevelItemStates[Enabled], TMT_TEXTCOLOR, LColorRef);
          Font.Color := LColorRef;
          DrawThemeBackground(ThemeServices.Theme[teMenu], Handle, MENU_BARITEM,
            MenuStates[(odHotLight in State), (odSelected in State)], ARect, nil);
        end
        else
        begin
      ...
      begin
        DrawThemeBackground(ThemeServices.Theme[teMenu], Handle,
      ...
      GetThemePartSize(ThemeServices.Theme[teMenu], Handle, MENU_POPUPCHECK,
        CheckMarkStates[Enabled, RadioItem], nil, TS_TRUE, LCheckSize);
      GetThemeMargins(ThemeServices.Theme[teMenu], Handle, MENU_POPUPCHECK,
      ...
      GetThemePartSize(ThemeServices.Theme[teMenu], Handle,
        MENU_POPUPGUTTER, 0, nil, TS_TRUE, LSize);
      GetThemeMargins(ThemeServices.Theme[teMenu], Handle,
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            StretchDraw(LBitmapRect, FBitmap);
          end;
        end;
      end;
  ...
  begin
    with ACanvas do
    begin
      if (WinXP and WinXPFlatMenus) or (WinXP and ThemeServices.ThemesEnabled) then
      begin
        if (odSelected in State) or (odHotLight in State) then
        begin
...
begin
  ...
  if WinXP and ThemeServices.ThemesAvailable then
...
{$IFEND}
  ...
  begin
    ...
    GetThemePartSize(LTheme, Handle, MENU_POPUPCHECK,
      CheckMarkStates[Enabled, RadioItem], nil, TS_TRUE, GlyphSize);
    ...
    begin
      ...
      GetThemePartSize(LTheme, Handle, MENU_BARITEM, MBI_NORMAL, nil, TS_TRUE, LSize);
      Inc(Height, LSize.cy);
    end
    else
    begin
      ...
      GetThemeMargins(LTheme, Handle, MENU_POPUPCHECK,
        CheckMarkStates[Enabled, RadioItem], TMT_CONTENTMARGINS, nil, LMargins);
      ...
      GetThemePartSize(LTheme, Handle, MENU_POPUPGUTTER, 0, nil, TS_TRUE, LSize);
      Inc(Width, LSize.cx);
      GetThemeMargins(LTheme, Handle, MENU_POPUPGUTTER, 0, TMT_SIZINGMARGINS, nil, LMargins);
      Inc(Width, LMargins.cxLeftWidth + LMargins.cxRightWidth);

      // Add margins for menu item
      GetThemeMargins(LTheme, Handle, MENU_POPUPITEM, MPI_NORMAL, TMT_SIZINGMARGINS, nil, LMargins);
      Inc(Width, LMargins.cxLeftWidth + LMargins.cxRightWidth);

      // Add size of potential submenu glyph
      GetThemePartSize(LTheme, Handle, MENU_POPUPSUBMENU, MSM_NORMAL, nil, TS_TRUE, LSize);
      Inc(Width, LSize.cx);
...
{$IFEND}    
    ...
    DoDrawVistaText(Self, ACanvas, Text, R, False, DrawStyle);
...
begin
  Result := False;
  IterateMenus({$IFNDEF CLR}@{$ENDIF}UpdateItem, MenuItem.FMerged, MenuItem);
...
{$ELSE}
      LCategory := TCharacter.GetUnicodeCategory(ACaption[I]);
...
begin
  ...
  if ((Kind = fkCommand) and (Value = Item.Command)) or
    ((Kind = fkHandle) and Assigned(Item.FHandle) and (Value = Integer(Item.FHandle.Value))) or
...
function IsAltGRPressed: boolean;
  ...
  type
    ...
    KbdLayerDescriptorFunc = function : PKBDTABLES;
  const
    KLLF_ALTGR = $0001;
  ...
  var
    ...
    pKbdTable: PKBDTABLES;
    fKbdLayerDescriptor: KbdLayerDescriptorFunc;
  begin
    LayoutName[0] := #0;
    ...
    try
      ...
      begin
        ...
        try
          fKbdLayerDescriptor := GetProcAddress(DLLHandle, 'KbdLayerDescriptor');
          pKbdTable := fKbdLayerDescriptor;

          IsAltGRKeyLayout := (LoWord(pKbdTable.fLocaleFlags) and KLLF_ALTGR) = KLLF_ALTGR;
        finally
...
begin
  ...
  AccelTableBuilder.Groups := Groups;
